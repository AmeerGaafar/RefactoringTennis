...
extern "C" { ;
    char* tennis_score(int p1Score, int p2Score,char * score);
}
using namespace std;


void assertTennisScore(int player1Score, int player2Score, string correctResult)
{
    char computedScore[255]="";
    EXPECT_EQ(correctResult,tennis_score(player1Score,player2Score,computedScore));
}
